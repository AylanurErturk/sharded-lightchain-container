...
public class LightChainNode extends SkipNode implements LightChainRMIInterface {
	...
	private static final AtomicInteger starting = new AtomicInteger(0);
	...
	public LightChainNode(Parameters params, int RMIPort, String introducer, boolean isInitial) throws RemoteException {
		super(RMIPort, params.getMaxShards(), params.getLevels(), introducer);
		this.params = params;
		...
		if (isInitial) {
			...
			if (idx < shardCount) {
				int baseNum = Integer.parseInt(name, 2);
				...
				if (newHash.length() > params.getLevels()) {
					...
					newHash = "0".repeat(params.getLevels() - newHash.length()) + newHash;
				}
				name = newHash;
			}
			isInitial = true;
		...
		if (!isInitial) {
			int idx = 0;
			if (offset.get() == shardCount)
				offset.getAndSet(0);

			idx = offset.getAndIncrement();
		...
		logger.info(isInitial + " name: " + name + " num: " + numID + " offset: " + offset.get() + " shard ID: "
				+ super.getShardID());
		...
		if (isInitial) {
			...
			Simulation.shardInserted(this.getShardID());
			addPeerNode(peer);
	 ...
	 *
	 */
	public View updateView(int shardID) {
		try {
			...
			if (tList == null) {
				logger.debug(" transaction list empty!");
				return null;
			}
	...
	public Block mineAttempt() {
		try {
			...
			Block blk = getLatestBlock(this.getShardID());
			...
			if (tList == null || tList.size() < params.getTxMin()) {
				logger.error("Mining Failed: not enough transaction found: " + tList.size());
			...
			Block newBlk = new Block(
					...
					blk.getIndex() + 1,
					params.getLevels(),
					assignToShard(getNumID()), params.getMaxShards());
	 ...
	 */
	public void insertFlagNode(Block blk, int shardID) {
		logger.debug(
				"inserting Flag Node from block num id:" + blk.getNumID() + " to shard " + shardID + " with num ID "
						+ shardID);
		super.insertDataNode(shardID, blk.getHash(), shardID);
	}
	...
	public Transaction makeTransaction(String cont) {
		try {
			...
			int waitCount = 3;
			int sid = this.getShardID();
			while (!success && waitCount > 0) {

				Block lstBlk = getLatestBlock(sid);
				logger.debug(
				...
				logger.debug("numID of the owner: " + getNumID() + " Validating transaction with num id " + t.getNumID()
						+ " and shard id " + t.getShardID() + " Prev of the transaction: "
						+ Integer.parseInt(t.getPrev(), 2));
				boolean verified = validateTransaction(t);
				if (verified) {
					insertTransaction(t);
					logger.info("Transaction Successfully Added with name: " + t.getNameID());
					return t;
				} else {
					logger.debug("Transaction adding failed.");
				}
				waitCount--;
	...
	public void insertBlock(Block blk, String prevAddress) throws RemoteException {
		...
		if (prevAddress.equals(getAddress())) {
			...
			prevOwnerRMI.removeFlagNode(blk.getShardID());

			insertNode(blk);
			insertFlagNode(blk, blk.getShardID());
		}
	}
	...
	public Block insertGenesis() {
		...
		int index = 0;
		Block b = new Block(prev, getNumID(), getAddress(), index, params.getLevels(), 0, params.getMaxShards());
		logger.debug("Owner: " + getNumID() + " prev: " + prev);
		logger.debug("Inserting Genesis Block " + b.getNumID() + " to shard " + b.getShardID());
		insertNode(b);
		// insertFlagNode(b, b.getShardID());
		return b;
	}

	public void insertFlagsToShards(Block b) {
		insertFlagNode(b, b.getShardID());
	}
	 ...
	 */
	public Block getLatestBlock(int shardID) {

		try {
			logger.debug("searching for flag");
			logger.debug("shard id given: " + shardID);
			NodeInfo flag = searchByNumID(shardID);
			logger.debug("flag num from latest block: " + flag.getNumID() + " flag name " + flag.getNameID());

			logger.debug("searching for latest block");
			int num = Integer.parseInt(flag.getNameID(), 2);

			int mShards = params.getMaxShards();
			num = (num / mShards) * mShards + shardID;

			logger.debug("num from latest block: " + num + " targetShard: " + shardID);
	...
	public boolean validateBlock(Block blk) {
		try {
			...
			List<NodeInfo> validators = getValidators(blk.toString(), blk.getShardID(), blk.getOwner());
	...
	public boolean validateTransaction(Transaction t) throws RemoteException {
		...
		try {
			...
			// obtain validators
			List<NodeInfo> validators = getValidators(t.toString(), t.getShardID(), t.getOwner());
			...
			for (int i = 0; i < validators.size(); ++i) {
				...
				logger.debug("isAuth: " + signature.isAuth() + " isCorrect: " + signature.isCorrect() +
						" isSound: " + signature.isSound() + " has balance: " + signature.hasBalance());
	...
	public boolean isAuthenticated(Block blk) throws RemoteException {
		try {
			...
			String hash = hasher.getHash(blk.getPrev() + blk.getOwner() + sb.toString(), params.getLevels()); // old
																												// hash

			int maxShards = params.getMaxShards();
			int baseNum = Integer.parseInt(hash, 2);
			int ownerShard = Math.floorMod(blk.getOwner(), maxShards); // we want same shard with the owner
	...
	public boolean isConsistent(Block blk) throws RemoteException {
		try {
			...
			Block lstBlk = getLatestBlock(blk.getShardID());
	 ...
	 */
	public List<NodeInfo> getValidators(String str, int shardID, int owner) {
		try {
			...
			for (int i = 0; validFound < params.getAlpha() && i < 200; ++i) {
				...
				num = (num / maxShards) * maxShards + ownerShard; // so this stays (this is the existing num always)
	...
	public boolean isAuthenticated(Transaction t) throws RemoteException {
		try {
			...
			int mShards = params.getMaxShards();
			int baseNum = Integer.parseInt(hash, 2);
			int ost = Math.floorMod(t.getOwner(), mShards); // we want same shard with the owner
			int adjustedNum = (baseNum / maxShards) * mShards + ost;

			String newHash = Integer.toBinaryString(adjustedNum);

			while (newHash.length() < params.getLevels()) {
				newHash = "0" + newHash;
			}

			// return false if it was not generated properly
			if (!newHash.equals(t.getHash())) {
			...
			boolean verified = false;
			if (ownerPublicKey == null) {
				logger.debug("Owner public key null");
				return false;
			}
			...
			for (int i = 0; i < tSigma.size(); ++i) {
				...
				// true
				boolean is = digitalSignature.verifyString(newHash, tSigma.get(i), ownerPublicKey);
				if (is)
					verified = true;
			}
			if (verified == false) {
	...
	public PublicKey getOwnerPublicKey(int num) throws RemoteException {
		try {
			...
			int adjustedNum = (hashedKey / maxShards) * maxShards + owner.getShardID();

			// int adjustedNum = (hashedKey / 10) * 10 + assignToShard(num);
			...
			if (adjustedNum != num) {
				...
				logger.debug(" numID matches with the hash of pk!!");
			}
			return pk;
		} catch (NumberFormatException e) {
			e.printStackTrace();
	...
	public SimLog startSimSync(int numTransactions, int pace) {
		...
		logger.info("Starting: " + starting.getAndIncrement());
		for (int i = 0; i < numTransactions; i++) {
			try {
			...
			logger.info("Making Transaction ..." + i);
			makeTransaction(Util.getRandomString(135));
		...
		logger.info("returning simlog: " + completed.getAndIncrement());
		return simLog;
	}
