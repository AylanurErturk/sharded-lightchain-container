...
public class SkipNode extends UnicastRemoteObject implements RMIInterface {
    ...
    public SkipNode(NodeConfig config, String introducer, boolean isInitial) throws RemoteException {
        ...
        setLookup(Simulation.lt(shardID, config.getMaxLevels(), config.getMaxShards()));
     ...
     */
    public SkipNode(int RMIPort, int maxLevels, int maxShards, String introducer) throws RemoteException {
    ...
    public void setLookup(LookupTable lookup) {
        this.lookup = lookup;
    }
    ...
    public void insertNode(NodeInfo insertedNode) {
        try {
            ...
            if (isInserted || Simulation.getIsInserted(insertedNode.getShardID())) {
    ...
    public List<NodeInfo> getNodesWithNameID(String name) {
        ...
        try {
            ...
            if (ansNode == null || !ansNode.getNameID().equals(name)) {
                logger.error("getNodesWithNameID: No Node was found with the given nameID: " +name);
                return list;
            }
